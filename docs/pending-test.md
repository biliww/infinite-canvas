...
- 管理后台模型渠道新增 `image_tasks` 协议选项，公开配置会下发模型协议映射；选择 `image_tasks` 模型生图/图生图时前端请求 `/api/v1/image-tasks/*`，普通 `openai` 模型仍请求 `/api/v1/images/*`。
...
- 画布节点生成面板和画布助手输入框改为 `Enter` 换行、`Ctrl/Command + Enter` 提交，并在中文输入法组合态下避免误提交。
